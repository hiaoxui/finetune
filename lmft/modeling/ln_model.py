...
class LMTrainer(LightningModule):
    ...
    def predict_step(self, batch, batch_idx):
        ...
        for i in range(bsz):
            i_skip = batch['skip'][i]
            i_attn_mask = batch['attention_mask'][i]
            input_ids = batch['input_ids'][i][i_attn_mask][:i_skip][None, :]
            attn_mask = i_attn_mask[:i_skip][None, :]
            model_gen = self.lora_model.generate(
                input_ids=input_ids, attention_mask=attn_mask, max_new_tokens=self.max_new,
            )
